 ...
 *
 * thisclass was designed to be used with tables that use an auto resize mode
 ...
 *  or more of the other columns must decrease. Because ofthisthe resize mode
...
public class TableColumnAdjuster implements PropertyChangeListener, TableModelListener
{
	...
	public void adjustColumns() {
		...
		for (int i = 0; i < tcm.getColumnCount(); i++) adjustColumn(i);
	}
	...
	private int getColumnDataWidth(int column) {
		...
		for (int row = 0; row < table.getRowCount(); row++) {
			...
			if (preferredWidth >= maxWidth) break; // We've exceeded the maximum width, no need to check other rows
		}
	...
	private void updateTableColumn(int column, int width) {
		...
		if (isOnlyAdjustLarger) width = Math.max(width, tableColumn.getPreferredWidth());
	...
	public void restoreColumns() {
		...
		for (int i = 0; i < tcm.getColumnCount(); i++) restoreColumn(i);
	}
	...
	public void tableChanged(TableModelEvent e) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() { //  A cell has been updated
